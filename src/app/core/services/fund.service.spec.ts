...
describe('FundService', () => {
  ...
  function setupService(): void {
    ...
    service = new FundService(mockApi as unknown as MockFundsApiService);
  }
  ...
  it('debería cargar el catálogo desde la API mock y almacenarlo en caché', async () => {
    // Arrange
    ...
    expect(service.catalog()[1]).toMatchObject({
      id: 'FDO-ACCIONES',
      name: 'FIC BTG Pactual Renta Variable',
      minAmount: 250_000,
      category: 'FIC',
    ...
    // Caching behavior
    mockApi.getFunds.mockClear();
    const fundsAgain = await firstValueFrom(service.getFunds());
    expect(fundsAgain).toEqual(catalog);
    expect(mockApi.getFunds).not.toHaveBeenCalled();
  });

  it('muestra la lista de fondos disponibles como no suscritos', async () => {
    // arrange
    service.catalog.set(catalog);
    service.subscribedFunds.set([{ ...catalog[0], subscribedAmount: 125_000, subscribedAt: new Date() }]);

    // act
    const disponibles = service.availableFunds();

    // assert
    expect(disponibles).toHaveLength(1);
    expect(disponibles[0].id).toBe('FDO-ACCIONES');
  });

...
describe('DashboardComponent', () => {
  ...
  it('renders the empty state when the user has no active funds', async () => {
    ...
    expect(screen.getByText(/no tienes posiciones abiertas/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /descubrir oportunidades/i })).toHaveAttribute('href', '/funds');
  });
  ...
  it('renders active fund summary and investment totals', async () => {
    ...
    expect(screen.getByText(/posiciones activas/i)).toBeInTheDocument();
    expect(screen.getByText('FPV BTG Pactual Recaudadora')).toBeInTheDocument();
    expect(screen.getByText(/1 posiciones activas/i)).toBeInTheDocument();
    expect(screen.getAllByText(/125\.000/)).toHaveLength(2);
    expect(screen.getByRole('link', { name: /gestionar inversiones/i })).toHaveAttribute('href', '/funds');
  });
});